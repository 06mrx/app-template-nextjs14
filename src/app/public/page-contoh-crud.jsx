...
export default function Index() {
    ...
    const { data: datas, mutate, isLoading: datasLoading } = useSWR('/api/public/index?page=' + (searchParams.get('page') ? searchParams.get('page') : '') + '&search=' + (searchParams.get('search') ? searchParams.get('search') : ''), fetcher)
    // const { data: datas, mutate, isLoading: datasLoading } = useSWR('/api/public/index?page=', fetcher)
    ...
    const generteKey = () => {
        ...
        // return result;
        document.getElementById('key').value = 'KZM-' + result;
    }

    const handlePagination = index => {
        if (index.includes('Next')) {
            index = searchParams.get('page') ? parseInt(searchParams.get('page')) + 1 : 2;
        } else if (index.includes('Previous')) {
            index = searchParams.get('page') ? parseInt(searchParams.get('page')) - 1 : 1;
        }
        ...
        // ROUTER.push({
        //     pathname: '/public',
        //     query: { ...searchParams, page: parseInt(index) }
        // },
        //     undefined,
        //     {}
        // )
    ...
    const handleSearch = event => {
        ...
        filterTimeout = setTimeout(() => {
            // ROUTER.push({
            //     pathname: '/public',
            //     // query: { ...ROUTER.query, page: searchParams.get('page') ? searchParams.get('page') : '', search: event }
            // },
            //     undefined,
            //     {}
            // )
            ROUTER.push(`/public?page=${searchParams.get('page') ? searchParams.get('page') : ''}&search=${event}`);
    ...
    return <>
        ...
        <NewPageLayout title={'Dasbor'} hasBackURL={false}>
            ...
            <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
                ...
                <input type="text" className="w-full input input-sm bg-gray-300 border border-green-100 mb-2" placeholder="Cari" defaultValue={searchParams.get('search') ? searchParams.get('search') : ''} onChange={(event) => handleSearch(event.target.value)} />
                <Table isCompact>
                    <thead>
                        <tr>
                            <th>No</th>
